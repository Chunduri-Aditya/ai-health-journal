# === app.py ===
# Journal Agent — Flask local UI backend
...
from .journal_gate import OFF_TOPIC_MESSAGE, classify_journal_relevance
...
from .providers.roles import RoleModels, resolve_role_models
...
# ── Flask app ─────────────────────────────────────────────────────────────────
# Templates and static files are in src/web/ after reorganization
import os as _os
_current_dir = _os.path.dirname(_os.path.abspath(__file__))
app = Flask(__name__,
           template_folder=_os.path.join(_current_dir, 'web', 'templates'),
           static_folder=_os.path.join(_current_dir, 'web', 'static'))
...
def analyze_entry():
    ...
    roles = _active_role_models()
    model             = data.get("model", roles.generator)
    ...
    if getattr(cfg, "journal_relevance_gate", True):
        ...
        verdict = classify_journal_relevance(
            journal_entry,
            provider=_provider if use_llm else None,
            cfg=cfg if use_llm else None,
            use_llm=use_llm,
        )
        if not verdict.relevant:
            logging.info(
                "Relevance gate blocked entry category=%s source=%s",
                verdict.category,
                verdict.source,
            )
            return jsonify({
                "error": OFF_TOPIC_MESSAGE,
                "relevance": {
                    "relevant": False,
...
def suggest_prompt():
    try:
        ...
        # Cloud backends use resolve_role_models; local uses Ollama selection.
        prompt_model = roles.prompt
...
if __name__ == "__main__":
    ...
    # Prefer FLASK_PORT / PORT so macOS AirPlay (Control Center on :5000)
    # does not steal the lab UI. Default remains 5000 for older docs/scripts.
    _port = int(os.getenv("FLASK_PORT") or os.getenv("PORT") or "5000")
    app.run(
